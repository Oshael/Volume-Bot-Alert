...
## Conservative Monitored Refactor Plan

This is the currently agreed execution order for the next frontend memory pass.

Scope for this pass:
- focus on the monitored/dashboard state flow first
- do not refactor PumpFun yet
- optimize without changing intended bot behavior

### Ordered plan

1. Add metrics and behavior checks.
2. Refactor the monitored flow so it becomes the single source of truth and the other monitored-derived views only reflect that shared token state.
3. Validate behavior before continuing.
4. After the monitored source-of-truth refactor is stable, attack larger structural deduplication.

### What step 1 means

Before changing the monitored state model, add temporary checks that let us compare before vs after behavior.

Checklist:
- log monitored token counts before and after each refresh
- log recent and old-week routed counts before and after refresh
- log whether monitored addresses differ unexpectedly between old and new paths
- log alert candidate counts during refresh cycles
- keep memory-oriented measurement enabled while refactoring

### What step 2 means

The monitored refactor should move toward:
- one shared monitored token record per address
- derived monitored views reflecting the same token objects instead of carrying separate full copies where possible
- less full-state reconstruction inside:
  - `applyMonitoredDashboard()`
  - `rebuildTrackedState()`
  - `deriveAgeBuckets()`

Important constraint:
- keep public behavior the same while changing internal structure

### What step 3 must validate

Do not continue to broader deduplication unless these remain stable:
- monitored counts
- recent counts
- old-week counts
- manual token behavior
- blocklist behavior
- starred token behavior
- alert triggering behavior
- pagination/search/sort behavior

### What step 4 means

Only after the monitored source-of-truth work is stable:
- reduce larger duplicated structures
- convert duplicated token collections into lighter derived/indexed forms
- remove old compatibility paths only after parity is confirmed

### Guardrails

Because this work touches the core state model, use these rules:
- change one internal layer at a time
- preserve current UI and API contracts first
- prefer temporary parallel validation over big-bang rewrites
- treat behavioral parity as more important than local code elegance
