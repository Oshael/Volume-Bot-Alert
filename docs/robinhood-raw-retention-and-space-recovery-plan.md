...
| Reorg e reclassificação | Stage 244 grava disposições append-only; reclassificação usa evidência sem raw quando o bloco canônico local ainda existe. O rollback marca `unknown` órfão sem raw, mas exige raw para provar cobertura/replay de projeções e aborta com `archive_required` se a faixa atravessar dia `dropped` | As duas tabelas não tornam o raw descartável; falta recuperação comprovada das projeções após poda e de eventos cujo bloco canônico local também saiu. |
