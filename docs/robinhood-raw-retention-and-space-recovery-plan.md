...
O desenho para substituir a tabela monolítica, manter as quatro FKs e descartar partições expiradas está em [Retenção física de `robinhood_chain_events`](robinhood-chain-events-physical-retention-plan.md). Ele é um projeto de esquema separado dos pilotos de `DELETE` abaixo.
