...
O atalho de clipboard fica incorporado à extremidade direita da busca. Após autenticar,
o frontend consulta a permissão e, quando necessário, mostra um aviso próprio; somente o
botão `Allow` desse aviso aciona o prompt nativo do navegador. Depois da concessão, cada
clique normal dentro do site fornece o gesto necessário para inspecionar silenciosamente
o clipboard, sem polling. Texto vazio, maior que 120 caracteres ou sem formato de endereço
suportado é descartado localmente antes de qualquer request, e a mesma identidade não é
resolvida novamente. Um contrato reconhecido substitui o ícone por imagem e ticker
clicáveis, que abrem o gráfico expandido; negação de permissão, troca de sessão e
resoluções substituídas não retêm conteúdo copiado indevidamente.
...
por hora em páginas ordenadas por token, com uma transação por página; somente o
bucket ausente ou anterior ao estado live entra na página, e o snapshot diário só
é atualizado quando a observação live avança. `ROBINHOOD_HOLDER_SNAPSHOT_INTERVAL_MS`
...
live recuperam leases expiradas separadamente para manter o claim de `pending`
servido pelo índice parcial, com lotes de até 100 itens. O source agrupa RPC
canônico, carrega signed-origin em uma consulta e materializa cada lote em uma
transação; falhas individuais continuam isoladas e retry-safe. Enquanto todas
as lanes receberem um lote cheio, o próximo ciclo é agendado imediatamente.
`ROBINHOOD_FRESH_WALLET_LIVE_LANES` permite 1–16 lotes paralelos e deve ficar em
1 no live estável; até 12 lanes são admitidas temporariamente contra Archive
privado durante catch-up. A concorrência máxima quatro permanece como fallback
por item. Cinco rodadas totalmente falhas abrem por padrão um circuit
...
`ROBINHOOD_RPC_MIN_INTERVAL_MS=0` com até 12 lanes; não use throttle zero nem
múltiplas lanes contra RPC público. Nesse catch-up privado,
`ROBINHOOD_FRESH_WALLET_RPC_SUB_BATCH_SIZE=50` reduz o fan-out HTTP sem remover
nenhuma verificação individual; o padrão público permanece 10. `lastResult`
separa `claimMs`, `evidenceMs`,
`persistMs` e `fallbackMs` e informa throughput agregado em `itemsPerSecond`.
...
apply muda o token de `shadow` ou `live` para `backfilling` com CAS, preservando
baseline e journal; `shadow` ainda sem `live_through_block` também pode retomar do
seu cursor persistido quando não há cauda aplicada. O backfill relê a lacuna desde
`backfill_next_block`, a telemetria soma `baselineRequeues` e um token antes `live`
publica invalidação até concluir o novo handoff. Sem uma dessas barreiras seguras ou
com receipts indisponiveis, o evento fica deferido sem falso `drifted`.
...
ajustados por `ROBINHOOD_HOLDER_DRIFT_REPAIR_MAX_REPLAY_BLOCKS` e
`ROBINHOOD_HOLDER_DRIFT_REPAIR_MAX_TOTAL_REPLAY_BLOCKS`; não há hard cap
operacional além da exigência técnica de inteiro positivo seguro. Alterar o
limite nao muda o comportamento read-only sem a flag de confirmacao.
...
Durante o rewind canônico, esse executor restaura os batches em ordem reversa,
remove o raw da faixa órfã e recua somente o cursor `live`. Se o ancestral cortar
um batch, o preimage é reancorado nele antes do replay do prefixo; ausência de
preimage para qualquer transfer elegível falha fechado e aborta todo o rewind.
...
leitura executa seis sondas independentes: transfer `unknown` sem evidência
...
é `candidate`: falta prova local, sem inferir que houve reorg. A sexta sonda
confere o marcador do checkpoint LIVE `unified_transfer_v1` e a cobertura
