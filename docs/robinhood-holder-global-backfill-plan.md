...
2. Validar schema e criar campanha em dry-run/preview. Antes de `AUTO_START`, o
   ...
   so resetados depois da revisao do dry-run. Tails `shadow` validos, mas acima da
   janela de receipts, usam `npm run robinhood:holder-tail-requeue` antes do scan.
