...
describe('Catalog routes', () => {
  ...
  it('keeps a young high-mcap token monitored when a source reports a suspicious launch-floor downgrade', async () => {
    await db.query('DELETE FROM admin_blocked_tokens WHERE address = $1', [VALID_ADDR]);
    await db.query('DELETE FROM token_market_buckets_1m WHERE token_address = $1', [VALID_ADDR]);
    await db.query('DELETE FROM token_catalog WHERE address = $1', [VALID_ADDR]);

    const tokenCreatedAt = Date.now() - (20 * 60 * 1000);
    const stalePairAddress = 'AKqqZBcod9cA8eWq43g27vTm3Qg2iXu9vY57WWQXYqsp';
    ...
    try {
      ...
      const downgraded = await tokenCatalog.applyEvaluationResult(VALID_ADDR, {
        evaluationSource: 'dexscreener',
        eligibilityState: 'dex-low-activity',
        eligibleForMonitoring: false,
        suppressedReason: 'low_activity_24h',
        monitorPriority: 'low',
        nextEvaluationAt: new Date(Date.now() + 180000),
        pairAddress: stalePairAddress,
        pairUrl: `https://dexscreener.com/solana/${stalePairAddress}`,
        mcap: 30355.83,
        price: 0.00003035583,
        tokenCreatedAt,
        vol5m: 0,
      ...
      assert.equal(Number(downgraded.last_mcap), 460123);
      assert.equal(Number(downgraded.last_price), 0.000460123);
      assert.equal(downgraded.eligible_for_monitoring, true);
      assert.equal(downgraded.eligibility_state, 'gmgn-high');
      assert.equal(downgraded.suppressed_reason, null);
      assert.equal(downgraded.monitor_priority, 'high');
      assert.equal(downgraded.last_pair_address, stalePairAddress);
    } finally {
      await db.query('DELETE FROM admin_blocked_tokens WHERE address = $1', [VALID_ADDR]).catch(() => {});
      await db.query('DELETE FROM token_market_buckets_1m WHERE token_address = $1', [VALID_ADDR]).catch(() => {});
      await db.query('DELETE FROM token_catalog WHERE address = $1', [VALID_ADDR]).catch(() => {});
    }
  });

  it('uses recent bucket highs to protect young tokens already clamped at the launch floor', async () => {
    await db.query('DELETE FROM admin_blocked_tokens WHERE address = $1', [VALID_ADDR]);
    await db.query('DELETE FROM token_market_buckets_1m WHERE token_address = $1', [VALID_ADDR]);
    await db.query('DELETE FROM token_catalog WHERE address = $1', [VALID_ADDR]);

    const tokenCreatedAt = Date.now() - (20 * 60 * 1000);
    const stalePairAddress = 'CSU2S5Z5hFLHT2qEamGRdtqmQBFZG69sT8Aajdp8Cmzj';
    const bucketTs = new Date(Date.now() - 60000);
    bucketTs.setUTCSeconds(0, 0);
    ...
    try {
      ...
      await db.query(
        `INSERT INTO token_market_buckets_1m (
         ...
         VALUES ($1, $2, $3, 73432.50, 137082.00, 30376.37, 30376.37, 22, 'gmgn')`,
        [VALID_ADDR, bucketTs, stalePairAddress]
      );

      const clamped = await tokenCatalog.applyEvaluationResult(VALID_ADDR, {
        evaluationSource: 'gmgn',
        eligibilityState: 'gmgn-normal',
        eligibleForMonitoring: true,
        suppressedReason: null,
      ...
      assert.equal(Number(clamped.last_mcap), 137082);
      assert.equal(clamped.eligible_for_monitoring, true);
      assert.equal(clamped.eligibility_state, 'gmgn-high');
      assert.equal(clamped.suppressed_reason, null);
      assert.equal(clamped.monitor_priority, 'high');
      assert.equal(clamped.last_pair_address, stalePairAddress);
    } finally {
      await db.query('DELETE FROM admin_blocked_tokens WHERE address = $1', [VALID_ADDR]).catch(() => {});
      await db.query('DELETE FROM token_market_buckets_1m WHERE token_address = $1', [VALID_ADDR]).catch(() => {});
      await db.query('DELETE FROM token_catalog WHERE address = $1', [VALID_ADDR]).catch(() => {});
