...
describe('Robinhood pool liquidity event cursor', () => {
  ...
  it('indexes only unfinished captures for bounded frontier reads', () => {
    const sql = stage150.STATEMENTS.join('\n');
    const group = SCHEMA_GROUPS.find(({ key }) => (
    ...
    assert.equal(group.repair, 'node src/utils/db-init-stage150.js');
  ...
  it('rewinds explicitly and stops before unfinished canonical outbox work', async () => {
    let call = 0;
    const repository = createRobinhoodPoolLiquidityEventCursorRepository({ database: {
      async query(sql) {
        call += 1;
