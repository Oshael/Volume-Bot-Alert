...
async function cleanup() {
  ...
  await db.query(`UPDATE ${stage237.PROGRESS_TABLE} SET
    ...
    target_block_hash=NULL, target_event_kind=NULL, target_captured_at=NULL,
    scanned=0, inserted=0, completed_at=NULL, updated_at=NOW()
...
before(async () => {
  ...
  await stage237.init({ database: db, closePool: false });
  await stage238.init({ database: db, closePool: false });
  await cleanup();
});
...
test('backfill resumes by key, repairs missing shadow state and proves batch parity', async () => {
  ...
  const beyondTarget = await db.query(`SELECT 1 FROM ${stage236.STATE_TABLE}
    WHERE transaction_hash=$1 AND log_index=$2`, [AFTER_TX, LOG_INDEX]);
  assert.equal(beyondTarget.rowCount, 0);
