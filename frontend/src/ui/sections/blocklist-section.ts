...
export function renderBlocklistSection(state: AppState, controller: AppController) {
  ...
  section.innerHTML = `
    ...
    <div class="blocklist-inline-tags">${state.data.blocklist.map((item) => `
      <span class="blocklist-tag">${item.label || item.address.slice(0, 8)}<button type="button" class="blocklist-tag-remove" data-action="remove-blocked" data-address="${item.address}" ${state.ui.busy ? 'disabled' : ''}>×</button></span>
    `).join('')}</div>
