...
function parseArgs(argv = []) {
  ...
  return Object.freeze({
    ...
    confirmations: integer(value('--confirmations='), 12, 0, 1000, '--confirmations'),
  });
}
...
async function buildCanary(options, deps = {}) {
  ...
  const rpcTimeoutMs = Number(env.ROBINHOOD_HOLDER_LIVE_RPC_TIMEOUT_MS
    || env.ROBINHOOD_RPC_TIMEOUT_MS || 15_000);
  const [legacy, canonical] = await Promise.all([
    resolveRobinhoodHolderLiveSource({
      sourceMode: 'rpc', providerName: 'robinhood-holder-canary-legacy', rpcTimeoutMs,
  ...
  return createRobinhoodCanonicalHolderCanary({
    readiness: deps.readiness || createRobinhoodCanonicalHolderAudit({
      database, confirmations: options.confirmations,
    }),
    legacySource: legacy.reader, canonicalSource: canonical.reader,
  });
}
...
if (require.main === module) main().then((report) => {
  ...
  console.error('Robinhood canonical holder canary failed:', error.message);
  process.exitCode = 1;
