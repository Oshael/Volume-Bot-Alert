...
/** Stage 163 - exact RPC trace provenance for factory-created Robinhood tokens. */
  ...
  `DO $migration$
   BEGIN
     ...
     ALTER TABLE robinhood_token_attributions
       ...
       CHECK (source IN ('blockscout', 'rpc_direct', 'rpc_trace', 'launchpad_event'));
     ALTER TABLE robinhood_token_attributions
     ...
     ALTER TABLE robinhood_token_attributions
       ADD CONSTRAINT robinhood_token_attributions_provenance_check CHECK (
         ...
         OR (source = 'rpc_trace' AND attribution_block IS NOT NULL
             AND attribution_block >= 0 AND attribution_tx_hash IS NOT NULL
             AND attribution_tx_hash ~ '^0x[0-9a-f]{64}$'
             AND attribution_factory_address IS NOT NULL
             AND attribution_factory_address ~ '^0x[0-9a-f]{40}$')
         OR (source = 'launchpad_event' AND attribution_block IS NOT NULL
             AND attribution_block >= 0 AND attribution_tx_hash IS NOT NULL
             AND attribution_tx_hash ~ '^0x[0-9a-f]{64}$'
             AND attribution_factory_address IS NOT NULL
             AND attribution_factory_address ~ '^0x[0-9a-f]{40}$')
...
async function init(options = {}) {
  try {
    ...
    console.log('Stage 163 Robinhood RPC trace provenance created successfully');
  } finally {
...
if (require.main === module) init().catch((error) => {
  console.error('Failed to create Stage 163:', error.message);
  process.exitCode = 1;
});
