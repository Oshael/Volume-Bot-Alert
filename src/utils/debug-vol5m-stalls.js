  ...
  const { rows } = await db.queryWithStatementTimeout(
    `WITH eligible AS (
     ...
     SELECT
       ...
       COUNT(*) FILTER (WHERE source IN ('user-watchlist', 'user-manual'))::int AS watchlist_count,
       COUNT(*) FILTER (WHERE source NOT IN ('user-watchlist', 'user-manual') AND COALESCE(last_vol_24h, 0) < 5000)::int AS low_activity_auto_count,
