/**
 * Stage 149 - Online frontier index for unfinished Robinhood head captures.
 *
 * Liquidity seed/runtime reads need the oldest pending, leased, or blocked
 * capture across discovery and market without scanning terminal history.
 * Run with: node src/utils/db-init-stage149.js
 */
...
async function removeInvalidIndex() {
  ...
  console.log(`Stage 149: removing interrupted invalid index ${INDEX_NAME}...`);
...
async function init(options = {}) {
  ...
  try {
    ...
    console.log('Stage 149: building unfinished processing frontier index...');
    await db.query(CREATE_STATEMENT);
    await assertIndexReady();
    console.log('Stage 149 Robinhood processing frontier index created successfully');
  } catch (error) {
    console.error('Failed to create Stage 149 processing frontier index:', error.message);
    process.exitCode = 1;
    throw error;
  } finally {
