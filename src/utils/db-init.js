...
-- Social identities (Phase 3 foundation)
...
-- Token-gated wallet access foundation
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_user_wallets_wallet_address ON user_wallets(wallet_address);
CREATE UNIQUE INDEX IF NOT EXISTS idx_user_wallets_user_id ON user_wallets(user_id);
CREATE INDEX IF NOT EXISTS idx_user_wallets_user_linked ON user_wallets(user_id, linked_at DESC);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_wallet_auth_challenges_nonce_hash ON wallet_auth_challenges(nonce_hash);
CREATE INDEX IF NOT EXISTS idx_wallet_auth_challenges_wallet_expires ON wallet_auth_challenges(wallet_address, expires_at DESC);
CREATE INDEX IF NOT EXISTS idx_wallet_auth_challenges_cleanup ON wallet_auth_challenges(expires_at, consumed_at);
