...
/** Stage 164 - exact Blockscout internal creation provenance for Robinhood tokens. */
  ...
  `DO $migration$
   BEGIN
     ...
     ALTER TABLE robinhood_token_attributions
       ...
       CHECK (source IN (
         'blockscout', 'blockscout_internal', 'rpc_direct', 'rpc_trace', 'launchpad_event'
       ));
     ALTER TABLE robinhood_token_attributions
...
async function init(options = {}) {
  try {
    ...
    console.log('Stage 164 Robinhood Blockscout internal provenance created successfully');
  } finally {
...
if (require.main === module) init().catch((error) => {
  console.error('Failed to create Stage 164:', error.message);
  process.exitCode = 1;
});
