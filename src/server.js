...
const robinhoodHolderIntelligenceWorker = require('./services/robinhood-holder-intelligence-worker');
...
const robinhoodBundleRedistributionLiveWorker = require(
  './services/robinhood-bundle-redistribution-live-worker'
);
...
const ROBINHOOD_HOLDER_INTELLIGENCE_LEASE_KEY = 'robinhood-holder-intelligence-worker';
...
const ROBINHOOD_BUNDLE_REDISTRIBUTION_LIVE_LEASE_KEY =
  'robinhood-bundle-redistribution-live-worker';
...
app.get('/api/admin/ws-status', authenticate, requireAdmin, async (req, res) => {
  ...
  res.json({
    ...
    robinhoodHolderIntelligenceWorker: robinhoodHolderIntelligenceWorker.getStatus(),
    ...
    robinhoodBundleRedistributionLiveWorker:
      robinhoodBundleRedistributionLiveWorker.getStatus(),
...
function startRobinhoodWalletClassificationWorkerGroup() {
  ...
  if (config.robinhoodTokenDeploymentWorker.enabled) {
    startLockedWorker(
      'robinhood-wallet-classification', ROBINHOOD_TOKEN_DEPLOYMENT_LEASE_KEY,
      'Robinhood token deployment LIVE worker',
      () => robinhoodTokenDeploymentWorker.start(config.robinhoodTokenDeploymentWorker),
      { metadataProvider: () => ({ telemetry: robinhoodTokenDeploymentWorker.getStatus() }) }
    );
  }
  ...
  if (config.robinhoodInsiderShadowWorker.enabled) {
    startLockedWorker(
      'robinhood-wallet-classification', ROBINHOOD_INSIDER_SHADOW_LEASE_KEY,
      'Robinhood INSIDER shadow worker',
      () => robinhoodInsiderShadowWorker.start(config.robinhoodInsiderShadowWorker),
      { metadataProvider: () => ({ telemetry: robinhoodInsiderShadowWorker.getStatus() }) }
    );
  }
  if (config.robinhoodFirstBuyLiveWorker.enabled) {
    startLockedWorker(
      ...
      }), { metadataProvider: () => ({ telemetry: robinhoodFirstBuyLiveWorker.getStatus() }) }
    );
  }
  if (config.robinhoodLaunchAnchorLiveWorker.enabled) {
    startLockedWorker(
      'robinhood-wallet-classification', ROBINHOOD_LAUNCH_ANCHOR_LIVE_LEASE_KEY,
      'Robinhood launch-anchor LIVE worker',
      () => robinhoodLaunchAnchorLiveWorker.start(config.robinhoodLaunchAnchorLiveWorker),
      { metadataProvider: () => ({ telemetry: robinhoodLaunchAnchorLiveWorker.getStatus() }) }
    );
  }
  if (config.robinhoodBundleFundingLiveWorker.enabled) {
    startLockedWorker(
  ...
  if (config.robinhoodBundleRedistributionLiveWorker.enabled) {
    startLockedWorker(
      'robinhood-wallet-classification', ROBINHOOD_BUNDLE_REDISTRIBUTION_LIVE_LEASE_KEY,
      'Robinhood BUNDLED redistribution shadow worker',
      () => robinhoodBundleRedistributionLiveWorker.start(
        config.robinhoodBundleRedistributionLiveWorker
      ), { metadataProvider: () => ({
        telemetry: robinhoodBundleRedistributionLiveWorker.getStatus(),
      }) }
    );
  }
  if (config.robinhoodFreshWalletLiveWorker.enabled) {
    startLockedWorker(
      'robinhood-wallet-classification', ROBINHOOD_FRESH_WALLET_LIVE_LEASE_KEY,
      'Robinhood FRESH wallet LIVE worker', () => robinhoodFreshWalletLiveWorker.start({
        ...config.robinhoodFreshWalletLiveWorker,
        rpcOptions: config.robinhoodIngestionWorker,
      }), { metadataProvider: () => ({ telemetry: robinhoodFreshWalletLiveWorker.getStatus() }) }
    );
  }
...
function startRobinhoodHolderWorkerGroup() {
  ...
  if (config.robinhoodHolderIntelligenceWorker.enabled) {
    startLockedWorker(
      'robinhood-holders', ROBINHOOD_HOLDER_INTELLIGENCE_LEASE_KEY,
      'Robinhood holder intelligence worker',
      () => robinhoodHolderIntelligenceWorker.start(
        config.robinhoodHolderIntelligenceWorker
      ),
      { metadataProvider: () => ({ telemetry: robinhoodHolderIntelligenceWorker.getStatus() }) }
    );
  }
...
async function shutdownGracefully(signal = 'SIGTERM') {
  ...
  try {
    await Promise.all([
      ...
      robinhoodBundleRedistributionLiveWorker.stop(),
      ...
      robinhoodHolderIntelligenceWorker.stop(),
