  ...
  async function readNextRange(input = {}) {
    ...
    try {
      ...
      const stateResult = await client.query(
        `WITH processing AS (
           SELECT MIN(checkpoint_block) AS checkpoint_block
             FROM robinhood_head_capture_cursors
            WHERE chain=$1 AND stream IN ('discovery', 'market')
           HAVING COUNT(*)=2 AND COUNT(checkpoint_block)=2
         ), pending AS (
           SELECT block_number
           ...
           SELECT capture.checkpoint_block AS journal_through,
                  ...
                  CASE WHEN processing.checkpoint_block IS NULL THEN NULL
                       WHEN pending.block_number IS NULL THEN processing.checkpoint_block
                       ELSE pending.block_number-1 END AS safe_head
