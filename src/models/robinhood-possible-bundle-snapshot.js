...
function normalizeSnapshot(snapshot, observedAt) {
  ...
  if (state.ruleVersion !== RULE_VERSION || state.evidenceVersion !== EVIDENCE_VERSION
      || state.sourceKind !== 'seed' || state.status !== 'ready') {
  ...
  const normalizedState = Object.freeze({ ...state,
    sourceRunId: positive(state.sourceRunId, 'sourceRunId'),
...
function frontierDecision(current, state) {
  ...
  if (current.through_block_number === state.throughBlockNumber
      && current.through_block_hash !== state.throughBlockHash) {
...
function createRobinhoodPossibleBundleSnapshotRepository(options = {}) {
  ...
  async function replaceSnapshot(input) {
    ...
    try {
      ...
      assertLineage(lineage, state);
      const current = (await client.query(
        `SELECT evidence_version, source_run_id::text, lookback_blocks::text,
                minimum_value_wei::text, through_block_number::text, through_block_hash
      ...
      ) VALUES ($1, $2, $3, $4, 'ready', $5, 'seed', $6::bigint, $7::bigint,
        $8::numeric, $9::bigint, $10, $11::timestamptz)
      ON CONFLICT (chain, token_address, rule_version) DO UPDATE SET
        ...
        source_run_id = EXCLUDED.source_run_id, lookback_blocks = EXCLUDED.lookback_blocks,
      ...
      [CHAIN, state.tokenAddress, state.ruleVersion, state.evidenceVersion,
        state.statusReason, state.sourceRunId, state.lookbackBlocks, state.minimumValueWei,
        state.throughBlockNumber, state.throughBlockHash, state.observedAt]);
...
module.exports = { createRobinhoodPossibleBundleSnapshotRepository,
  __private: { assertLineage, frontierDecision, normalizeSnapshot } };
