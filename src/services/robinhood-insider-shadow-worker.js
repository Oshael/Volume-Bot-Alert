const { createRobinhoodInsiderShadowRunner } = require('./robinhood-insider-shadow-runner');
...
function createRobinhoodInsiderShadowWorker(deps = {}) {
  const schedule = deps.schedule || setTimeout;
  const cancelSchedule = deps.cancelSchedule || clearTimeout;
  const logger = deps.logger || console;
  const runner = deps.runner || (deps.runnerFactory || createRobinhoodInsiderShadowRunner)();
  if (typeof runner?.runBatch !== 'function') {
    throw new TypeError('INSIDER shadow worker runner is invalid');
  }
  ...
  async function execute() {
    ...
    try {
      ...
      status.lastError = Object.freeze({
        code: error.code || 'insider_shadow_error',
        message: String(error.message || error).slice(0, 500),
      });
      logger.warn('[RobinhoodInsiderShadowWorker] Tick failed:', error.message);
      return null;
    } finally {
