  ...
  return createLoopRuntime({
    ...
    async execute(options) {
      ...
      const worker = (deps.workerFactory || createRobinhoodBackfillEnrichmentWorker)({
        adapterFactory: async (claims) => {
          const seedPools = await repository.listActivePoolsByIdentities(claims);
          return (deps.adapterFactory || createRobinhoodBackfillEnrichmentAdapter)({
            seedPools,
            rpcClient,
            rpcProvider: 'drpc',
            timestampProvider,
