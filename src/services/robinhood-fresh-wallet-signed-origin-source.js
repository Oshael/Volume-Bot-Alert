...
function combineEvidence(input, canonical, row) {
...
function createRobinhoodFreshWalletSignedOriginSource(options = {}) {
  ...
  async function loadRows(walletAddresses) {
    const unique = [...new Set(walletAddresses)];
    ...
    const { rows } = await database.query(`SELECT cursor.origin_block::text,
      ...
      FROM UNNEST($2::varchar[]) AS wallet(wallet_address)
      CROSS JOIN robinhood_wallet_signed_origin_cursors cursor
      LEFT JOIN robinhood_wallet_signed_origins origin
        ON origin.chain = cursor.chain AND origin.wallet_address = wallet.wallet_address
      WHERE cursor.chain = $1 AND cursor.stream = 'live'
        AND cursor.lifecycle_state IN ('running', 'caught_up')`, [
      CHAIN, unique,
    ]);
    return new Map(rows.map((row) => [row.wallet_address, row]));
  }

  async function readEvidence(input = {}) {
    const canonical = await canonicalSource.readCanonicalEvidence(input);
    const rows = await loadRows([canonical.firstBuy.walletAddress]);
    return combineEvidence(input, canonical, rows.get(canonical.firstBuy.walletAddress));
  }

  async function readEvidenceBatchResults(inputs = []) {
    if (typeof canonicalSource.readCanonicalEvidenceBatch !== 'function') {
      throw new TypeError('FRESH canonical RPC batch source is required');
    }
    const canonicals = await canonicalSource.readCanonicalEvidenceBatch(inputs);
    if (!Array.isArray(canonicals) || canonicals.length !== inputs.length) {
      throw new Error('FRESH canonical RPC batch is incomplete');
    }
    const rows = await loadRows(canonicals.map(({ firstBuy }) => firstBuy.walletAddress));
    return Object.freeze(canonicals.map((canonical, index) => {
      try {
        return Object.freeze({ evidence: combineEvidence(inputs[index], canonical,
          rows.get(canonical.firstBuy.walletAddress)) });
      } catch (error) { return Object.freeze({ error }); }
    }));
  }

  return Object.freeze({ sourceKind: 'live', readEvidence, readEvidenceBatchResults });
}

module.exports = { createRobinhoodFreshWalletSignedOriginSource,
  __private: { combineEvidence } };
