...
function evaluate(input) {
  const { phase, maxCaptureLag, minDiscovery, minMarket } = input.options;
  ...
  if (phase === 'preflight') {
    ...
    const forbidden = count(leases.canary.metadata?.runtime?.rpcGuard?.forbiddenAttempts);
...
function createRobinhoodCanonicalHeadCanaryAudit(deps = {}) {
  ...
  async function inspect(options = {}) {
    ...
    const [state, leaseResult, parity] = await Promise.all([
      database.query(
      ...
      database.query(
        `SELECT lease_key, lease_until>NOW() AS active, heartbeat_at, lease_until, metadata
           FROM worker_leases WHERE lease_key=ANY($1::varchar[])`, [Object.values(LEASE_KEYS)]
      ),
      candidates.getParitySummary(),
    ]);
    ...
    return evaluate({
      ...
      queue: row && {
        ...
        first_pending: row.first_pending,
      },
