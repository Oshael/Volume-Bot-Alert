const candidateBuilder = require('./pumpfun-combo-confirmation-candidates');
const { evaluatePumpfunComboConfirmationSignal } = require('./pumpfun-combo-confirmation-signal');
const detectionStore = require('../models/pumpfun-combo-confirmation-detection');
...
function buildPassedCandidate(candidate, result) {
  return {
    ...
    blastAlertTriggeredAt: candidate.blastAlertTriggeredAt,
    fastAlertTriggeredAt: candidate.fastAlertTriggeredAt,
    score: result.score,
...
function buildNewDetection(candidate, result, now) {
  ...
  const comboMcap = toFiniteNumber(evidence.blastAlertMcap);
  const comboTriggeredAt = toIsoOrNull(candidate.blastAlertTriggeredAt || candidate.currentBucketAt) || now.toISOString();
  ...
  return {
    ...
    migrationStartedAt: candidate.migrationStartedAt,
    comboTriggeredAt,
    alertTriggeredAt: comboTriggeredAt,
    comboMcap,
    alertMcap: comboMcap,
    score: result.score,
    reason: result.reason,
    evidenceAtTrigger: evidence,
    evidenceAtAlert: evidence,
    latestMcapSinceAlert: comboMcap,
    latestBucketAt: comboTriggeredAt,
    maxMcapSinceAlert: comboMcap,
    maxMcapBucketAt: comboTriggeredAt,
    maxXSinceAlert: 1,
...
function upsertDetection(candidate, result, now) {
  ...
  existing.evidenceAtTrigger = {
    ...(result.evidence || {}),
    sourceEvidence: candidate.sourceEvidence || {},
  };
  existing.evidenceAtAlert = existing.evidenceAtTrigger;
...
function getTrackedDetections() {
  return Array.from(trackedDetections.values())
    .sort((a, b) => String(b.alertTriggeredAt || b.comboTriggeredAt).localeCompare(String(a.alertTriggeredAt || a.comboTriggeredAt)));
}
...
function pruneTrackedDetections(now) {
  ...
  for (const [address, detection] of trackedDetections.entries()) {
    const alertAt = toDate(detection.alertTriggeredAt || detection.comboTriggeredAt);
...
async function refreshTrackedOutcomes(now) {
  ...
  const outcomes = await candidateBuilder.listPumpfunComboConfirmationOutcomesSinceAlert(
    tracked.map((detection) => ({
      address: detection.address,
      alertTriggeredAt: detection.alertTriggeredAt || detection.comboTriggeredAt,
      alertMcap: detection.alertMcap ?? detection.comboMcap,
    })),
    { now }
  );
...
async function evaluateOnce(options = {}) {
  const candidates = await candidateBuilder.listPumpfunComboConfirmationCandidates({
  ...
  for (const candidate of candidates) {
    const result = evaluatePumpfunComboConfirmationSignal(candidate.signalInput);
    if (result.passes) {
...
async function runOnce(options = {}) {
  ...
  try {
    ...
    console.error('[PumpFunComboConfirmationDryRun] Evaluation failed:', err.message);
    return null;
  }
}
...
function start(options = {}) {
  ...
  if (!settings.dryRun) {
    ...
    console.warn('[PumpFunComboConfirmationDryRun] Not started: alert emission is not implemented yet');
    return;
  }
  ...
  void runOnce();
  schedule();
  console.log(`[PumpFunComboConfirmationDryRun] Started intervalMs=${settings.intervalMs} candidateLimit=${settings.candidateLimit}`);
}
